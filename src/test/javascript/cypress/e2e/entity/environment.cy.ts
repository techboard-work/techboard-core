...
describe('Environment e2e test', () => {
  ...
  const environmentSample = { name: 'Branding', code: 'structure GB open-source', color: 'yellow', level: 69830 };
  ...
  describe('new Environment page', () => {
    ...
    it('should create an instance of Environment', () => {
      ...
      cy.get(`[data-cy="code"]`).type('e-services').should('have.value', 'e-services');

      cy.get(`[data-cy="color"]`).type('orange').should('have.value', 'orange');

      cy.get(`[data-cy="level"]`).type('16936').should('have.value', '16936');

      cy.get(`[data-cy="link"]`).type('Card Senior').should('have.value', 'Card Senior');
