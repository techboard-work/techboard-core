...
describe('Activity e2e test', () => {
  ...
  const activitySample = { name: 'pricing sensor Bedfordshire', startedOn: '2023-03-19T20:13:05.065Z' };
  ...
  describe('new Activity page', () => {
    ...
    it('should create an instance of Activity', () => {
      ...
      cy.get(`[data-cy="startedOn"]`).type('2023-03-20T10:56').blur().should('have.value', '2023-03-20T10:56');

      cy.get(`[data-cy="finishedOn"]`).type('2023-03-20T08:00').blur().should('have.value', '2023-03-20T08:00');

      cy.get(`[data-cy="link"]`).type('killer Factors Heights').should('have.value', 'killer Factors Heights');

      cy.get(`[data-cy="impediment"]`).should('not.be.checked');
      cy.get(`[data-cy="impediment"]`).click().should('be.checked');
