...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'receivedOn'> & {
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      receivedOn: event.receivedOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      receivedOn: restEvent.receivedOn ? dayjs(restEvent.receivedOn) : undefined,
    };
  }
